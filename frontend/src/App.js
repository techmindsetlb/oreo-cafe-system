...
function POSPage(){
  ...
  const filteredItems=selCat?menuItems.filter(i=>i.category_id===selCat&&i.is_available):[];
    ...
    <div className="pos-left">
      ...
      {!cur.orderType?(
        ...
        <>
          ...
          ):(<div className="item-grid">{filteredItems.map(item=><div key={item.id} className="item-card" onClick={()=>addToCart(item,activePerson)}>
            <span className="item-name">{item.name}</span><span className="item-price">${parseFloat(item.price).toFixed(2)}</span>{item.preparation_time>0&&<span className="item-time">⏱ {item.preparation_time}m</span>}
          </div>)}</div>)}
        </>
      )}
    </div>
...
if(alreadyInUse)return alert('This station is already in use in another tab.');
  ...
  return(<div className="gaming-wrap">
    ...
    <div className="gaming-left">
      ...
      {(curGTab.session||curGTab.cart.length>0)&&<><h3 className="section-lbl">Add Items</h3><div className="item-grid">{menuItems.map(item=><div key={item.id} className="item-card" onClick={()=>addItem(item)}><span className="item-name">{item.name}</span><span className="item-price">${parseFloat(item.price).toFixed(2)}</span></div>)}</div></>}
...
function MenuPage(){const{t}=useLang();const emp=JSON.parse(localStorage.getItem('employee')||'{}');const mgmt=can(emp.role,'menu_manage');const[cats,setCats]=useState([]);const[items,setItems]=useState([]);const[tab,setTab]=useState('items');const[showForm,setShowForm]=useState(false);const[editing,setEditing]=useState(null);const[form,setForm]=useState({name:'',description:'',price:'',category_id:'',preparation_time:'5',is_available:1});const[catForm,setCatForm]=useState({name:'',description:'',icon:''});const[showCatIconPicker,setShowCatIconPicker]=useState(false);useEffect(()=>{load();},[]);const load=async()=>{try{const[c,i]=await Promise.all([api.menu.getCategories(),api.menu.getItems()]);setCats(Array.isArray(c)?c:[]);setItems(Array.isArray(i)?i:[]);}catch{}};const si=async e=>{e.preventDefault();try{const d={...form,price:parseFloat(form.price),category_id:form.category_id?parseInt(form.category_id):null};if(editing)await api.menu.updateItem(editing.id,d);else await api.menu.createItem(d);setShowForm(false);setEditing(null);setForm({name:'',description:'',price:'',category_id:'',preparation_time:'5',is_available:1});load();}catch(ex){alert(ex.message);}};const sc=async e=>{e.preventDefault();try{await api.menu.createCategory(catForm);setCatForm({name:'',description:'',icon:''});load();}catch(ex){alert(ex.message);}};const se=item=>{setEditing(item);setForm({name:item.name,description:item.description||'',price:item.price.toString(),category_id:item.category_id?.toString()||'',preparation_time:item.preparation_time?.toString()||'5',is_available:item.is_available});setShowForm(true);};
