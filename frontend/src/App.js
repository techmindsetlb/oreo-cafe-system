...
function buildNetProfitHTML(data, from, to, lang, t){
  const summaryRows = [
    ['Revenue', '$' + parseFloat(data.total_revenue || 0).toFixed(2), 'style="color:#22c55e"'],
    ['Total Cost', '$' + parseFloat(data.total_cost || 0).toFixed(2), 'style="color:#ef4444"'],
    ['Net Profit', '$' + parseFloat(data.total_net_profit || 0).toFixed(2), parseFloat(data.total_net_profit || 0) >= 0 ? 'style="color:#22c55e;font-weight:bold"' : 'style="color:#ef4444;font-weight:bold"']
  ];
  const summaryHtml = '<div style="display:flex;gap:16px;flex-wrap:wrap;margin:12px 0">' + summaryRows.map(function(r){
    return '<div style="background:#f5f5f5;padding:10px 16px;border-radius:6px;min-width:120px"><div ' + r[2] + ' style="font-size:20px;font-weight:bold">' + r[1] + '</div><div style="font-size:10px;color:#000;margin-top:2px">' + r[0] + '</div></div>';
  }).join('') + '</div>';
  const itemRows = (data.items || []).map(function(it, i){
    const profitColor = parseFloat(it.net_profit) >= 0 ? '#22c55e' : '#ef4444';
    return '<tr><td>' + (i+1) + '</td><td><strong>' + it.name + '</strong></td><td>' + (it.category || '-') + '</td><td>' + it.total_quantity + '</td><td>$' + parseFloat(it.total_revenue).toFixed(2) + '</td><td>$' + parseFloat(it.total_cost).toFixed(2) + '</td><td style="color:' + profitColor + ';font-weight:bold">$' + parseFloat(it.net_profit).toFixed(2) + '</td><td>' + (parseFloat(it.total_revenue) > 0 ? ((parseFloat(it.net_profit) / parseFloat(it.total_revenue)) * 100).toFixed(1) : '0') + '%</td></tr>';
  }).join('');
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Net Profit Report</title><style>body{font-family:Arial,sans-serif;font-size:12px;padding:20px;color:#000;max-width:900px;margin:0 auto}h1{font-size:18px;margin-bottom:2px}h2{font-size:13px;margin:16px 0 6px;color:#000;border-bottom:1px solid #ddd;padding-bottom:4px}table{width:100%;border-collapse:collapse;margin-bottom:16px}th{background:#1c110a;color:#fff;padding:6px 10px;text-align:left;font-size:11px}td{padding:5px 10px;border-bottom:1px solid #f0f0f0;font-size:11px}tr:hover td{background:#fafafa}@media print{body{padding:0}}</style></head><body><h1>☕ ' + (t.appName || 'Cafe') + ' — Net Profit Report</h1><p style="color:#000;font-size:11px">' + from + ' → ' + to + ' · Printed ' + new Date().toLocaleString() + '</p>' + summaryHtml + '<h2>Items Breakdown</h2><table><thead><tr><th>#</th><th>Item</th><th>Category</th><th>Qty</th><th>Revenue</th><th>Cost</th><th>Net Profit</th><th>Margin</th></tr></thead><tbody>' + itemRows + '</tbody></table><script>window.onload=function(){window.print();setTimeout(function(){window.close()},1200)}</script></body></html>';
}
...
function AccountingPage(){
  ...
  const[qrImage,setQrImage]=useState(null);const[savingQR,setSavingQR]=useState(false);const[netProfitData,setNetProfitData]=useState(null);const[netFrom,setNetFrom]=useState(()=>{const d=new Date();d.setDate(d.getDate()-30);return d.toISOString().split('T')[0];});const[netTo,setNetTo]=useState(()=>new Date().toISOString().split('T')[0]);
  ...
  useEffect(()=>{if(netFrom&&netTo)api.reports.getNetProfit(netFrom,netTo).then(setNetProfitData).catch(()=>{});},[netFrom,netTo]);
  ...
  return(<div className="page">
    <div className="page-hd"><h2>💰 {t.accounting}</h2><div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',alignItems:'center'}}>
      <div className="tab-group">{['overview','income','expenses','cashflow','netprofit','qr'].map(tb=><button key={tb} className={tab===tb?'tab active':'tab'} onClick={()=>setTab(tb)}>{tb==='overview'?t.overview:tb==='income'?'Financial Summary':tb==='expenses'?t.expenses:tb==='cashflow'?t.cashFlow:tb==='netprofit'?'Net Profit':t.qrPayment}</button>)}</div>
      {activeData&&tab!=='netprofit'&&<button className="btn-ghost" onClick={()=>printWin(buildReportHTML(activeData,periodLabel,lang,{...t,appName}))}>🖨️ {t.printReport}</button>}
    </div></div>
    {(tab!=='expenses'&&tab!=='cashflow'&&tab!=='qr'&&tab!=='netprofit')&&<div style={{display:'flex',gap:'0.75rem',marginBottom:'1rem',flexWrap:'wrap',alignItems:'flex-end'}}>
    ...
    {tab==='netprofit'&&<><div style={{display:'flex',gap:'0.75rem',marginBottom:'1rem',flexWrap:'wrap',alignItems:'flex-end'}}>
    <div className="field" style={{maxWidth:160,marginBottom:0}}><label>{t.fromDate}</label><input type="date" value={netFrom} onChange={e=>setNetFrom(e.target.value)}/></div>
    <div className="field" style={{maxWidth:160,marginBottom:0}}><label>{t.toDate}</label><input type="date" value={netTo} onChange={e=>setNetTo(e.target.value)}/></div>
    {netProfitData&&<button className="btn-ghost" style={{marginLeft:'auto'}} onClick={()=>{printWin(buildNetProfitHTML(netProfitData,netFrom,netTo,lang,{...t,appName}))}}>🖨️ {t.printReport}</button>}
    </div>
    {netProfitData&&<div className="stats-grid">
    <div className="stat-card sc-green"><div className="stat-val">{parseFloat(netProfitData.total_revenue||0).toFixed(2)}</div><div className="stat-lbl">💵 Revenue</div></div>
    <div className="stat-card sc-red"><div className="stat-val">{parseFloat(netProfitData.total_cost||0).toFixed(2)}</div><div className="stat-lbl">📤 Total Cost</div></div>
    <div className="stat-card" style={{borderColor:parseFloat(netProfitData.total_net_profit||0)>=0?"var(--green)":"var(--red)"}}><div className="stat-val" style={{color:parseFloat(netProfitData.total_net_profit||0)>=0?"var(--green)":"var(--red)"}}>{parseFloat(netProfitData.total_net_profit||0).toFixed(2)}</div><div className="stat-lbl">📈 Net Profit</div></div>
    </div>}
    {netProfitData&&netProfitData.items&&netProfitData.items.length>0&&<div className="rpt-section"><h3>📦 Items Breakdown</h3><div className="data-table"><table><thead><tr><th>#</th><th>Item</th><th>Category</th><th>Qty</th><th>Revenue</th><th>Cost</th><th>Net Profit</th><th>Margin</th></tr></thead><tbody>
    {netProfitData.items.map((it,i)=><tr key={i}><td>{i+1}</td><td><strong>{it.name}</strong></td><td>{it.category||'-'}</td><td>{it.total_quantity}</td><td>{parseFloat(it.total_revenue).toFixed(2)}</td><td>{parseFloat(it.total_cost).toFixed(2)}</td><td style={{color:parseFloat(it.net_profit)>=0?'var(--green)':'var(--red)',fontWeight:700}}>{parseFloat(it.net_profit).toFixed(2)}</td><td>{parseFloat(it.total_revenue)>0?((parseFloat(it.net_profit)/parseFloat(it.total_revenue))*100).toFixed(1):0}%</td></tr>)}
    </tbody></table></div></div>}
    {!netProfitData&&<p className="muted" style={{padding:'2rem 0',textAlign:'center'}}>Select a date range and wait for data to load.</p>}
